...
<html>
	<head>
		...
		<script type="text/javascript">
			function addbt(){
				$("#flagi").val("add");
				$("#guname").textbox('setValue','');
				$("#gpwd").textbox('setValue','');
				$("#dlg").dialog('open');
			}
			function updbt(id){
				$.ajax({
				url:'<%=basePath%>t_admin/getdata',
				type:'post',
				dataType:'json',
				data:{"id":id},
				success:function(data){	
					...
					$("#idi").val(id);
					$("#dlg").dialog('open');
					$("#guname").textbox('setValue',ob.uname);
					$("#gpwd").textbox('setValue',ob.pwd);
				}
			});
		}
		//保存
		function gltj(){
			var uname=$("#guname").textbox('getValue');
			var pwd=$("#gpwd").textbox('getValue');
			var id=$("#idi").val();
			var flag=$("#flagi").val();
			if(uname==""||pwd==""){
				alert('管理员信息不完整。');
			...
			if("add"==flag){
				...
				$.ajax({
					url:'<%=basePath%>t_admin/upd',
					type:'post',
					data:{"id":id,"uname":uname,"pwd":pwd},
					dataType:'json',
					success:function(data){
						alert(data.msg);
					}
		...
		//取消
		function rst(){
			window.location.href="<%=basePath%>t_admin/apage?pnum="+${pagenum};
		}
		//查询
		function cktj(){
			$("#ckf").submit();
...
<body>	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td height="30"><table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="STYLE7" style="padding-left: 50px;">管理员信息管理</td>
					<td style="padding-right:10px;"><div align="right" style="padding-right: 50px;">
						<form id="ckf" action="<%=basePath%>t_admin/ck" method="post">
						<table border="0" align="right" cellpadding="0" cellspacing="0">
							<tr>
								<td class="STYLE3">
									账号：<input id="ckuname" name="uname" class="ckinput easyui-textbox"/>
								</td>
								<td width="60">
							...
							<td><table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#c9c9c9">
								...
								<c:forEach var="a" items="${alist }">
									<tr>
										...
										<td height="22" bgcolor="#FFFFFF"><div align="center"><span class="STYLE3">${a.uname }</span></div></td>
										<td height="22" bgcolor="#FFFFFF"><div align="center"><span class="STYLE3">${a.pwd }</span></div></td>
										...
										<td><table width="100%" border="0" cellspacing="0" cellpadding="0">
											<tr>
												...
												<td><table border="0" align="right" cellpadding="0" cellspacing="0">
													<tr>
														<td width="40"><a href="<%=basePath%>t_admin/apage?pnum=1"><img src="images/first.gif" width="37" height="15"/></a></td>
														<td width="45"><a href="<%=basePath%>t_admin/apage?pnum=${pagenum-1}"><img src="images/back.gif" width="37" height="15"/></a></td>
														<td width="45"><a href="<%=basePath%>t_admin/apage?pnum=${pagenum+1}"><img src="images/next.gif" width="37" height="15"/></a></td>
														<td width="40"><a href="<%=basePath%>t_admin/apage?pnum=${pagenums}"><img src="images/last.gif" width="37" height="15"/></a></td>
				...
				<div id="dlg" class="easyui-dialog" title=" " data-options="closable:false,modal:true,closed:true" style="width:100%;max-width:400px;padding:30px 60px;">
					<form id="ff" method="post">
						<div style="margin-bottom:20px">
							<input id="guname" class="easyui-textbox" name="guname" style="width:100%"  data-options="label:'账号:'"/>
						</div>
						<div style="margin-bottom:20px">
							<input id="gpwd" class="easyui-textbox" name="gpwd" style="width:100%"  data-options="label:'密码:'"/>
						</div>
					</form>
