	...
	<head>
		...
		<script type="text/javascript">
			function addbt(){
				...
				$("#gt_train_id").combobox('setValue','0');
				$("#dlg").dialog('open');
			}
			function updbt(id){
				$.ajax({
				url:'<%=basePath%>t_train_emp/getdata',
				type:'post',
				dataType:'json',
				data:{"id":id},
				success:function(data){	
					...
					$("#idi").val(id);
					$("#dlg").dialog('open');
					$("#gt_train_id").combobox('setValue',ob.t_train_id);
					$("#gt_dept_id").combobox('setValue',ob.t_dept_id);
					//根据部门，联动获取职位
					$.ajax({
		...
		function gltj(){
			...
			var t_train_id=$("#gt_train_id").combobox('getValue');
			var id=$("#idi").val();
			var flag=$("#flagi").val();
			if(t_staff_id=="0"||t_train_id=="0"){
				alert('员工培训信息不完整。');
				return;
			}
			if("add"==flag){
				$.ajax({
					url:'<%=basePath%>t_train_emp/add',
					type:'post',
					data:{"t_staff_id":t_staff_id,"t_train_id":t_train_id},
					dataType:'json',
					success:function(data){
						alert(data.msg);
					}
				...
				$.ajax({
					url:'<%=basePath%>t_train_emp/upd',
					type:'post',
					data:{"id":id,"t_staff_id":t_staff_id,"t_train_id":t_train_id},
					dataType:'json',
					success:function(data){
						alert(data.msg);
					}
		...
		//取消
		function rst(){
			window.location.href="<%=basePath%>t_train_emp/apage?pnum="+${pagenum};
		}
		//查询
		function cktj(){
			$("#ckf").submit();
...
<body>	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td height="30"><table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="STYLE7" style="padding-left: 50px;">员工培训管理</td>
					<td style="padding-right:10px;"><div align="right" style="padding-right: 50px;">
						<form id="ckf" action="<%=basePath%>t_train_emp/ck" method="post">
						<table border="0" align="right" cellpadding="0" cellspacing="0">
							<tr>
								<td class="STYLE3">
									培训：<select id="ckuname" name="t_train_id" class="ckinput easyui-combobox">
										<option value="0">所有培训</option>
										<c:forEach var="a" items="${t_trainlist }">
											<option value="${a.id }">${a.train_pxzt}</option>
										</c:forEach>
									</select>
								</td>
								<td width="60">
						...
						<tr>
							<td><table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#c9c9c9">
								<tr>
									...
									<td height="22" bgcolor="#FFFFFF"><div align="center"><strong><span class="STYLE1">培训主题</span></strong></div></td>
										...
										<td><table width="100%" border="0" cellspacing="0" cellpadding="0">
											<tr>
												...
												<td><table border="0" align="right" cellpadding="0" cellspacing="0">
													<tr>
														<td width="40"><a href="<%=basePath%>t_train_emp/apage?pnum=1"><img src="images/first.gif" width="37" height="15"/></a></td>
														<td width="45"><a href="<%=basePath%>t_train_emp/apage?pnum=${pagenum-1}"><img src="images/back.gif" width="37" height="15"/></a></td>
														<td width="45"><a href="<%=basePath%>t_train_emp/apage?pnum=${pagenum+1}"><img src="images/next.gif" width="37" height="15"/></a></td>
														<td width="40"><a href="<%=basePath%>t_train_emp/apage?pnum=${pagenums}"><img src="images/last.gif" width="37" height="15"/></a></td>
				...
				<div id="dlg" class="easyui-dialog" title=" " data-options="closable:false,modal:true,closed:true" style="width:100%;max-width:400px;padding:30px 60px;">
					<form id="ff" method="post">
						...
						<div style="margin-bottom:20px">
							<select id="gt_train_id" class="easyui-combobox" name="gt_train_id" style="width:100%"  data-options="label:'培训:'">
