...
<html>
	<head>
		...
		<script type="text/javascript">
			function addbt(){
				$("#flagi").val("add");
				$("#gdept_bmmc").textbox('setValue','');
				$("#gdept_cjsj").datebox('setValue','');
				$("#dlg").dialog('open');
			}
			function updbt(id){
				$.ajax({
				url:'<%=basePath%>t_dept/getdata',
				type:'post',
				dataType:'json',
				data:{"tbname":"t_dept","id":id},
				success:function(data){	
					var ob=data.ob;
					$("#flagi").val("upd");
					$("#idi").val(id);
					$("#dlg").dialog('open');
					$("#gdept_bmmc").textbox('setValue',gstr(ob.dept_bmmc));
					$("#gdept_cjsj").textbox('setValue',gstr(ob.dept_cjsj));
				}
			});
		}
		//保存
		function gltj(){
			var dept_bmmc=hstr($("#gdept_bmmc").textbox('getValue'));
			var dept_cjsj=hstr($("#gdept_cjsj").datebox('getValue'));
			var id=$("#idi").val();
			var flag=$("#flagi").val();
			if(dept_bmmc==""||dept_cjsj==""){
				alert('部门信息不完整。');
				return;
			}
			if("add"==flag){
				$.ajax({
					url:'<%=basePath%>t_dept/add',
					type:'post',
					data:{"dept_bmmc":dept_bmmc,"dept_cjsj":dept_cjsj},
					dataType:'json',
					success:function(data){
						alert(data.msg);
					}
				...
				$.ajax({
					url:'<%=basePath%>t_dept/upd',
					type:'post',
					data:{"id":id,"dept_bmmc":dept_bmmc,"dept_cjsj":dept_cjsj},
					dataType:'json',
					success:function(data){
						alert(data.msg);
					}
		...
		//取消
		function rst(){
			window.location.href="<%=basePath%>t_dept/apage?pnum="+${pagenum};
		}
		//查询
		function cktj(){
			$("#ckf").submit();
		...
		function delsbt(){
			...
			$.ajax({
				url:'<%=basePath%>t_dept/del',
				type:'post',
				dataType:'json',
				data:{"ids":ids,"tbname":"t_dept"},
				success:function(data){
					alert(data.msg);
					rst();
				}
...
<body>	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td height="30"><table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="STYLE7" style="padding-left: 50px;">部门信息管理</td>
					<td style="padding-right:10px;"><div align="right" style="padding-right: 50px;">
						<form id="ckf" action="<%=basePath%>t_dept/ck" method="post">
						<table border="0" align="right" cellpadding="0" cellspacing="0">
							<tr>
								<td class="STYLE3">
									部门名称：<input id="ckuname" name="dept_bmmc" class="ckinput easyui-textbox"/>
								</td>
								<td width="60">
						...
						<tr>
							<td><table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#c9c9c9">
								<tr>
									...
									<td height="22" bgcolor="#FFFFFF"><div align="center"><strong><span class="STYLE1">部门名称</span></strong></div></td>
									<td width="120px" height="22" bgcolor="#FFFFFF"><div align="center"><strong><span class="STYLE1">创建时间</span></strong></div></td>
								...
								<c:forEach var="a" items="${alist }">
									<tr>
										...
										<td height="22" bgcolor="#FFFFFF"><div align="center"><span class="STYLE3">${a.dept_cjsj }</span></div></td>
										...
										<td><table width="100%" border="0" cellspacing="0" cellpadding="0">
											<tr>
												...
												<td><table border="0" align="right" cellpadding="0" cellspacing="0">
													<tr>
														<td width="40"><a href="<%=basePath%>t_dept/apage?pnum=1"><img src="images/first.gif" width="37" height="15"/></a></td>
														<td width="45"><a href="<%=basePath%>t_dept/apage?pnum=${pagenum-1}"><img src="images/back.gif" width="37" height="15"/></a></td>
														<td width="45"><a href="<%=basePath%>t_dept/apage?pnum=${pagenum+1}"><img src="images/next.gif" width="37" height="15"/></a></td>
														<td width="40"><a href="<%=basePath%>t_dept/apage?pnum=${pagenums}"><img src="images/last.gif" width="37" height="15"/></a></td>
				...
				<div id="dlg" class="easyui-dialog" title=" " data-options="closable:false,modal:true,closed:true" style="width:100%;max-width:400px;padding:30px 60px;">
					<form id="ff" method="post">
						<div style="margin-bottom:20px">
							<input id="gdept_bmmc" class="easyui-textbox" name="gdept_bmmc" style="width:100%"  data-options="label:'部门名称:'"/>
						</div>
						<div style="margin-bottom:20px">
							<input id="gdept_cjsj" class="easyui-datebox" name="gdept_cjsj" style="width:100%"  data-options="label:'创建时间:'"/>
						</div>
					</form>
